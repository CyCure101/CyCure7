...
router.post('/register', (req, res) => {
    ...
    db.query(checkUser, [username, email], (err, results) => {
        ...
        db.query(insertUser, [username, email, password], (err, result) => {
            ...
            res.json({ success: true, message: 'User registered successfully', user: { id: result.insertId, username, email } });
        });
    });
});
...
router.get('/quizzes/:id/questions', (req, res) => {
    ...
    db.query(quizCheck, [quizId], (err, quizResults) => {
      ...
      SELECT q.id AS question_id, q.question_text, q.question_type,
             a.id AS answer_id, a.answer_text, a.is_correct, a.answer_order
      FROM questions q
    ...
    `;
        db.query(correctSql, [quizId, ...questionIds], (err, rows) => {
            ...
            db.query(insertAttempt, [req.session.userId, quizId, correctCount, totalQuestions], (err, result) => {
                ...
                if (graded.length === 0)
                    return res.json({ success: true, results: { attemptId, score: correctCount, totalQuestions, correctCount } });
...
router.get('/users/:id/attempts', (req, res) => {
    ...
    const sql = `
    SELECT ua.id AS attempt_id, ua.user_id, ua.quiz_id, q.title AS quiz_title, q.module_type,
           ua.score, ua.total_questions, ua.completed_at
